...
**Requirement:** Assign product lifecycle states only from dated evidence for the reviewed artifact and scope, with accountable ownership, limitations, and rollback or reversal considerations. Assign `production` only while the complete `production-candidate` boundary remains satisfied, accountable delegated approval is recorded, and current deployment evidence demonstrates that the exact approved artifact and configuration were successfully deployed into and are operating within the stated production environment and scope under the approved conditions. Readiness approval, deployment intent, or an earlier or different deployment does not establish this boundary; a failed, blocked, not-run, missing, stale, or different-artifact deployment or production-verification result prevents `production`. Assign `scaled-production` only while that complete `production` evidence boundary remains satisfied and after explicitly selecting the Site Reliability Engineering package and satisfying its complete [Scaling Strategy Standard](../disciplines/sre/standards/SCALING_STRATEGY_STANDARD.md): the overall scaling-strategy decision is `Verified`, every applicable scaling area is `Verified` with current representative evidence satisfying authorized criteria and a stated supported envelope, any `Applicable`, `NotRun`, `Blocked`, or `Failed` area prevents that state, and every `NotApplicable` area requires justification.

**Expected evidence:** Lifecycle transition record naming the prior and new state, artifact or revision, owner, decision, supporting evidence, limitations, and reversal considerations. A `production` transition also records the approval authority and date; exact artifact, source revision, configuration, production environment and operating scope; deployment record and completion time; successful deployment outcome; post-deployment health, monitoring, or representative operational verification; and rollback or recovery readiness. A `scaled-production` transition retains the production approval and all deployment evidence, records the SRE package selection, and links the complete scaling applicability matrix, overall `Verified` decision, decision authority, supported envelope, and evidence for every applicable area.
...
| `production` | The `production-candidate` evidence boundary remains satisfied; an accountable human with delegated authority approves production use; and current deployment evidence demonstrates that the exact approved artifact and configuration were successfully deployed into and are operating within the stated production environment and scope under the approved conditions. A failed, blocked, not-run, missing, stale, or different-artifact deployment or production-verification result prevents this state. |
| `scaled-production` | The `production` evidence boundary remains satisfied; the Site Reliability Engineering package is selected; and its complete Scaling Strategy Standard is satisfied: the overall scaling-strategy decision is `Verified`, and every applicable scaling area is `Verified` with current representative evidence satisfying authorized criteria and a stated supported envelope. Any `Applicable`, `NotRun`, `Blocked`, or `Failed` area prevents this state; every `NotApplicable` area has recorded justification. |
...
Before transition to `production`, retain the complete `production-candidate` boundary and accountable delegated approval, then demonstrate through current primary evidence that the exact approved artifact, source revision, and configuration were successfully deployed into and are operating within the stated production environment and operating scope under the approved conditions. Record the deployment identifier, start and completion time, outcome, owner, post-deployment health or monitoring verification, known limitations, and rollback or recovery readiness.

A production-readiness `Pass`, approval to deploy, deployment plan, configuration intent, or evidence from an earlier, staging, canary-only, different-artifact, or different-scope deployment does not establish the `production` state. A failed, blocked, `NotRun`, missing, stale, or different-artifact deployment or production-verification result keeps the product at its accurate earlier lifecycle state.
