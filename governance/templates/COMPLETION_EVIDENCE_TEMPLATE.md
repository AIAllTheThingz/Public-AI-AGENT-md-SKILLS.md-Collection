...
- Failed or indeterminate outcomes keyed by objective, with non-empty summaries and the same objective's complete failure-bearing retry ledger; actual-state reconciliation; and confirmation that authorization and recovery controls remain valid for consequential mutations:
...
For schema-backed JSON, embed each failure-bearing objective ledger under its matching `failedOrIndeterminateOutcomes` key and reserve the separate `retryLedger` field for success/evidence-only objectives. A rendered evidence table may present rows from both locations together.
