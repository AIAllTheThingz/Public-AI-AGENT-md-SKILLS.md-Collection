...
- One budget-consuming attempt is one consequential action ending after observable effects are reconciled with a Failed or Indeterminate result; each subsequent consequential action counts as a retry even within the same command sequence, plan, workflow, tool, agent, or strategy, while read-only discovery, reconciliation, and non-state-changing validation are non-consuming and Successful ends the objective or blocker.
