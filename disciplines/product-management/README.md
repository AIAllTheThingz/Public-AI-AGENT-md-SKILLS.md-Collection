...
When the explicitly selected Product Inception Lifecycle is used, a `production` state additionally requires accountable approval plus current primary evidence that the exact approved artifact and configuration were successfully deployed into and are operating within the stated production environment and scope. Candidate readiness, approval, deployment intent, or a different deployment does not establish production use.
