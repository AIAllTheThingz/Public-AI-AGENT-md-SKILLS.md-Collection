---
id: DISC-TPL-UX-EVIDENCE
title: User Experience Evidence Record Template
version: 0.1.0
status: baseline
---

# User Experience Evidence Record

## Context

- Product, journey, and interface version:
- Intended users, goals, tasks, and context:
- Requirements:
- Owner, researchers, and reviewers:
- Research state:
- Validation state:

Research and UX validation are independent claims. Record each state from its applicable standard; `NotRun` research must not be presented as completed UX validation, and validation evidence must not imply that user research occurred.

## Method and evidence

- Research evidence or rationale:
- Validation evidence or rationale:
- Method and research question:
- Participant/source boundaries and consent record:
- Environment and date:
- Findings, conflicting evidence, and limitations:
- Synthetic assumptions or simulated behavior:
- Accessibility evidence location:

## Results

| Journey/task | Finding or measure | Evidence | State | Decision | Owner |
|---|---|---|---|---|---|
| | | | | | |

- Checks not run and reasons:
- Excluded users or contexts:
- Unresolved risk and follow-up:
- Completion statement:
