...
Within a v2 retry ledger, each objective separates zero or more `priorUnresolvedSequences` from exactly one `currentSequence`. Only a sequence whose final attempt is failed or indeterminate with `reported-unresolved` may be preserved as a prior sequence; a current sequence after any prior sequence requires reset evidence for the prior stop/report, separate accountable authorization, and the material blocker or relevant scope or system-state change. A sequence may omit `attempts` only when it contains at least one successful `nonConsumingActions` entry, so evidence-only activity is recorded without fabricating a budgeted attempt. Failure reports are keyed by objective under `failedOrIndeterminateOutcomes`; every value carries non-empty outcome summaries and that objective's failure-bearing retry ledger. The separate `retryLedger` field contains only success/evidence-only objective ledgers. `status: "validated"` requires at least one reported `passed` validation, any reported `passed` validation requires at least one `Successful` action across either ledger location, and a reported `failed` validation requires at least one keyed failed-or-indeterminate outcome. `retry-authorized` requires the corresponding next retry to be present. Delegated work must record `boundariesPreserved: true`. Successful objective-clearing actions remain recorded at their current retry position and cannot be followed by another sequence for that objective.
