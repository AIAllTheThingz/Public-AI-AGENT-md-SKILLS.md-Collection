...
Within a v2 retry ledger, each objective separates zero or more `priorUnresolvedSequences` from exactly one `currentSequence`. Only a sequence whose final attempt is failed or indeterminate with `reported-unresolved` may be preserved as a prior sequence; a current sequence after any prior sequence requires reset evidence for the prior stop/report, separate accountable authorization, and the material blocker or relevant scope or system-state change. `retry-authorized` requires the corresponding next retry to be present. Non-empty failed or indeterminate outcomes require at least one objective ledger, every supplied objective ledger must contain an actual failed or indeterminate attempt, and delegated work must record `boundariesPreserved: true`. Successful objective-clearing actions remain recorded at their current retry position and cannot be followed by another sequence for that objective.
