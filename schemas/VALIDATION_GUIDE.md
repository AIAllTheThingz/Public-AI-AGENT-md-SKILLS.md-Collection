...
- For `status: "implemented"`, require a non-empty retry ledger and a `Successful` action with `terminalDisposition: "objective-completed"`. Preserve arbitrary fractional precision and normalize UTC offsets arithmetically so accepted calendar-boundary timestamps do not overflow the semantic validator.
