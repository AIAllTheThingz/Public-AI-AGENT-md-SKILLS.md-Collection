...
The `0.10.0` release was published on 2026-08-16 through the tag-driven workflow and is recorded under `publishedVersions` in release-state metadata. The next intended publication is `1.0.0-rc.1`; the release tools must continue to fail closed for the unpublished historical `0.9.0` boundary and for publication attempts inconsistent with `nextIntendedVersion`.
