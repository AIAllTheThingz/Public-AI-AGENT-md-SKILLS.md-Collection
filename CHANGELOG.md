...
- Added the breaking completion-report template package 0.3.0 and completion-result schema v2.0.0, which require current completion records to carry execution-discipline evidence; repository validation routes omitted versions to the current major, retained v1 records require explicit version identification there, successful objective-clearing actions remain ledgered at their retry position, executed validation results identify their exact retry-ledger objective and action, reset authorization is linked and chronologically bounded, and terminal dispositions prevent later actions. JSON Schema validates record structure and the completion semantic validator enforces exact validation-action correlation, cross-record chronology, and the full accepted RFC 3339 timestamp domain; existing template and schema consumers must migrate while historical or pinned v1 records remain supported.
- Required implemented completion claims to carry a non-empty retry ledger with a `Successful` objective-completing action, and made semantic RFC 3339 ordering exact across arbitrary fractional precision and UTC-normalized calendar boundaries.
...
- Updated the completion-report template and completion-result contract to require execution-discipline records, with rolling/current completion-result v2, structural and semantic validation, and preserved v1 historical compatibility.
...
- Existing completion-template and completion-result schema consumers must migrate new and current records to template package 0.3.0 and completion-result schema v2.0.0, populate execution-discipline fields, preserve repeatable per-objective retry sequences by separating prior sequences that actually ended `reported-unresolved` from exactly one current sequence, record successful non-consuming evidence-gathering actions as ledger rows without fabricating budgeted attempts, map each executed validation to one exact corresponding action in the exact objective ledger, link and timestamp reset authorization between adjacent sequences, keep pre-terminal evidence chronologically before the terminal attempt, prohibit actions after objective completion, require a corresponding retry for `retry-authorized`, keep failed or indeterminate outcomes and ledger attempts bidirectionally consistent while permitting separate success-only objective ledgers, run both structural schema and semantic validation, require delegated work to preserve its boundaries, retain v1 for historical records or pinned consumers, and obtain the required breaking-change review; do not rewrite historical v1 records in place.
