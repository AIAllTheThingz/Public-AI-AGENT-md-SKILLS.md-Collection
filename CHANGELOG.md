...
- Existing completion-template and completion-result schema consumers must migrate new and current records to template package 0.3.0 and completion-result schema v2.0.0, populate execution-discipline fields, preserve repeatable per-objective retry sequences with prior stop/report, separate authorization, and material-change evidence on every sequence after the first, retain v1 for historical records or pinned consumers, and obtain the required breaking-change review; do not rewrite historical v1 records in place.
