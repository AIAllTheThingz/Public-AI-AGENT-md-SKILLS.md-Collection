...
- Added the breaking completion-report template package 0.3.0 and completion-result schema v2.0.0, which require current completion records to carry execution-discipline evidence; repository validation routes omitted versions to the current major, retained v1 records require explicit version identification there, successful objective-clearing actions remain ledgered at their retry position, executed validation results identify their exact retry-ledger objective, reset authorization is linked and chronologically bounded, and `reported-unresolved` prevents subsequent retries. JSON Schema validates record structure and the completion semantic validator enforces cross-record objective and time-order invariants; existing template and schema consumers must migrate while historical or pinned v1 records remain supported.
