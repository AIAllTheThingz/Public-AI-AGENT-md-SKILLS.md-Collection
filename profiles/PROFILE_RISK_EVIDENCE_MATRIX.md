...
| Web Application | `moderate` | product requirements and traceability, user journeys and honest research state, server-side authorization model, client and server trust boundaries, separate WCAG 2.2 AA or stricter accessibility evidence |
...
| Desktop Application | `moderate` | product requirements and traceability, user journeys and honest research state, local data protection, installer and update integrity, privilege and elevation boundaries |
| Mobile Application | `moderate` | product requirements and traceability, user journeys and honest research state, secure storage, network trust and certificate behavior, permission minimization |
...
| Public Library | `moderate` | consumer outcomes, requirements and traceability, public API compatibility, supported runtimes and platforms, dependency minimization |
...
| Multi-Tenant SaaS | `high` | product requirements and traceability, tenant identity and isolation model, authorization at every object boundary, data partitioning and lifecycle, UX evidence when a tenant or operator interface applies |
...
| AI Agent Application | `high` | product requirements and traceability, tool authorization and allowlists, prompt and retrieved-content trust boundaries, human approval for consequential actions, UX evidence when people interact with the agent |
