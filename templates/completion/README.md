...
| `{{RETRY_LEDGER_ROWS}}` | One row per budget-consuming action and each subsequent successful objective-clearing action at its current retry position, grouped by objective and sequence with sequence-specific reset evidence and terminal disposition. |
